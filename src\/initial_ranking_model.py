...
def reranking_algo(db, id_col, title_col, base_col, ce_model):
    ...
    weighted_score = (0.5 * db_temp[base_col])
    ...
    if "final_good_score_max" in db_temp.columns:
        weighted_score += (0.3 * db_temp["final_good_score_max"])
        active_weight += 0.3

    if "final_bad_score_max" in db_temp.columns:
        bad_avoidance = (1 - db_temp["final_bad_score_max"])

        weighted_score += (0.2 * bad_avoidance)
        active_weight += 0.2
    ...
    db_temp["model_rank"] = db_temp["model_fit_score"].rank(ascending=True,method="min")
